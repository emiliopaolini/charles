...
template <int Digits=TOTAL_DIGITS,int Exponent=FRACTIONAL_DIGITS,class baseType=cnl::signed_multiprecision<Digits>>
...
template<class Archive,int Digits=TOTAL_DIGITS,int Exponent=FRACTIONAL_DIGITS,class baseType=cnl::signed_multiprecision<Digits>>
void save(Archive & archive, saturated_scaled_integer<Digits,Exponent,baseType> const & m){ 
  archive((float)m); 
}
